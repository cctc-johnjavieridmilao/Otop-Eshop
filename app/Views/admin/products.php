    ...
    <div class="az-content pd-y-20 pd-lg-y-30 pd-xl-y-40">
      <div class="container">
        <div class="az-content-body pd-lg-l-40 d-flex flex-column">
          ...
          <div class=" card">
              <div class=" card-body">
                    <button class="btn btn-outline-success" id="btn_add_product" style="display: none;">Add Products <span class="fas fa-plus"></span></button>
                    <br><br>
        ...
        <div class="modal-content modal-content-demo">
          ...
          <div class="modal-body">
            <div class="row">
                ...
                <div class="col-md-6">
                     <label>Category</label>
                     <select id="u_p_cat" class="form-control" readonly>
                        <option value=""> - </option>
                     </select>
                </div>
                <div class="col-md-6">
                     <label>Prize</label>
                     <input type="text" class="form-control" onblur="FormatAmount(this)" readonly onkeypress="return NumnbersOnly(event)" id="u_p_prize">
                </div>
                <div class="col-md-6">
                     <label>Stocks</label>
                     <input type="text" class="form-control" onkeypress="return NumnbersOnly(event)" id="u_p_stocks" readonly>
                </div>
                <div class="col-md-12">
                     <label>Description</label>
...
function DeleteProducts(product_id) {
  if(confirm('Are you sure you want to delete this product ?') == false) {
    ...
    $.post('<?= base_url('Admin/DeleteProduct') ?>', {product_id: product_id}, function(result) {
...
function GetProducts() {
      $.get('<?= base_url('Admin/GetProductAll') ?>',(result) => {
              ...
              var tr = $(`
                <tr>
                    ...
                    <td align="center" width="15%">
                        <button class="btn btn-outline-primary" onclick="UpdateProduct(${row.ProducID})"><span class="fas fa-search"></span></button>
                    </td>
                </tr>
              `);
...
function UpdateProductData() {
  ...
  $.post('<?= base_url('Admin/UpdateProductsData') ?>', {
          p_name: $('#u_p_name').val(),
          p_cat:  $('#u_p_cat').val(),
    ...
    $(function() {
      ...
      var myDropzone = new Dropzone("#myDropzone", {
          url: '<?= base_url('Admin/AddProducts') ?>',
          method: 'POST',
          paramName: "images",
        ...
        var UmyDropzone = new Dropzone("#UmyDropzone", {
          url: '<?= base_url('Admin/UpdateProducts') ?>',
          method: 'POST',
          paramName: "images",
          autoProcessQueue: false,
          uploadMultiple: true, // uplaod files in a single request
