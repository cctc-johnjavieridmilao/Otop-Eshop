      ...
      <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content modal-content-demo">
          ...
          <div class="modal-body">
            ...
            <div class="card">
                  <div class="card-body">
                    <h5 class=" card-title" style="float: left;">Delivery Address <a href="#" onclick="ShowModal('NewAddressModal')"><span class="fas fa-plus"></span></a></h5>
                    <table class="table table-striped">
      ...
      <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content modal-content-demo">
          <div class="modal-header">
            ...
            <button type="button" class="close" onclick="CloseModal('NewAddressModal')" aria-label="Close">
              <span aria-hidden="true">&times;</span>
            </button>
          </div>
          <div class="modal-body">
          ...
          <div class="modal-footer">
            <button type="button" onclick="CloseModal('NewAddressModal')" class="btn btn-outline-light">Close</button>
...
// function Addquantity(cartid, unit_price, product_id) {

//   var quantity = parseInt($('#load_cart_data').find('#tr_'+cartid).find('#quantity_'+cartid).val());

//   quantity++;

//   var total_price = (unit_price * quantity) + '.00';

//   $.post('<?= base_url('Home/UpdateQuantity') ?>', {cartid: cartid,quantity: quantity,total_price: total_price,product_id: product_id}, function(response) {
//     if(response.msg != 'success') {
//       swal.fire(response.msg, '', 'warning');
//       return false;
//     }
//     $('#load_cart_data').find('#tr_'+cartid).find('#total_price_row_'+cartid).text(total_price)
//     $('#load_cart_data').find('#tr_'+cartid).find('#quantity_'+cartid).val(quantity)

//     GetCart();
//   });
// }

// function Lessquantity(cartid, unit_price, product_id) {
//   var quantity = parseInt($('#load_cart_data').find('#tr_'+cartid).find('#quantity_'+cartid).val());
...
//   $.post('<?= base_url('Home/LesQuantity') ?>', {cartid: cartid,quantity: quantity,total_price: total_price}, function(response) {
...
function DeleteCartItem(cart_id) {
  ...
  $.post('<?= base_url('Home/DeleteCartItem') ?>', {cart_id: cart_id}, function(response) {
      if(response.msg == 'success') {
...
function AddToCart(product_id, prize) {
  $.post('<?= base_url('Home/AddCart') ?>', {product_id: product_id,prize: prize,quantity: parseInt($('#load_products').find('#quantity_'+product_id).val())}, function(response) {
       if(response.msg == 'success') {
            ...
            $('#load_vendors').append(`
              <div class="col-md-12">
                ...
                <div>
                    ...
                    <span class="badge badge-pill badge-primary" style="color: white;cursor: pointer">Followers (${row.followers_count})</span>
        ...
        $.post('<?= base_url('Home/CheckOut') ?>', {
          ...
          addressID: $('#load_address').find('.address_check:checked').val()
            ...
            if(response.msg == 'success') {
              ...
              setTimeout(() => {
                 window.location.href = '<?= base_url('Home/Pending') ?>';
              },2000);
            } else {
              swal.fire(response.msg, '', 'error');
            }
